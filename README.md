...
The project consist of a backend and a frontend structure. The Database and web part is based in Loopia where the domain name "haslerud.tech" got purchased by Even Johan Pereira Haslerud, so that the project could include a MySQL/MariaDB, UNIX (PHP 8.1/Apache 2.4) configuration, FTP user and SSL which we could include in the project to get it more secure. 
